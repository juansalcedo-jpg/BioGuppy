    ...
    formulario.addEventListener("submit", function (event) {
        ...
        .then(function (texto) {
            ...
            if (respuesta.tipo === "seguimiento") {
                mostrarSeguimiento(respuesta);
            }

            if (respuesta.tipo === "mortalidad") {
                mostrarMortalidad(respuesta);
            }

            if (respuesta.tipo === "tanques") {
                mostrarTanques(respuesta);
            }
        })
            ...
            respuesta.datos.forEach(function (fila) {
                html += `
                    <tr>
                        ...
                        <td>${escapar(fila.tanque)}</td>
                        <td>${escapar(fila.responsable)}</td>
                        <td>${escapar(fila.observaciones)}</td>
        ...
        if (respuesta.datos.length === 0) {
            ...
            respuesta.datos.forEach(function (fila) {
                html += `
                    <tr>
                        <td class="ps-4">${escapar(fila.fecha)}</td>
                        <td>${escapar(fila.zoocriadero)}</td>
                        <td>${escapar(fila.tanque)}</td>
                        <td>${escapar(fila.nacidos)}</td>
                        <td>${escapar(fila.muertos)}</td>
                        <td>${escapar(fila.responsable)}</td>
                        <td class="text-center">${mostrarEstado(fila.estado)}</td>
                    </tr>
                `;
            ...
            respuesta.datos.forEach(function (fila) {
                html += `
                    <tr>
                        ...
                        <td>${escapar(fila.tanque)}</td>
                        <td>${escapar(fila.tipo)}</td>
                        <td>${escapar(fila.capacidad)} L</td>
                        <td class="text-center">${mostrarEstado(fila.estado)}</td>
                    </tr>
                `;
            });
