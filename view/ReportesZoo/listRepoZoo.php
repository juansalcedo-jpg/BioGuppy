        ...
        <div class="card-header bg-white border-bottom py-3 px-4">
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    ...
                    <span class="text-muted fs-7">Define los criterios para procesar la información</span>
                </div>
            </div>
        </div>

        <div class="card-body p-4">
            <form id="formGenerarReporteZoo" action="<?php echo getUrl('ReportesZoo', 'ReportesZoo', 'generar', false, 'ajax'); ?>" method="POST">
                <div class="row g-4 align-items-end">

                    <!-- Tipo de Reporte (Zoocriadero) -->
                    <div class="col-12 col-xl-5">
                        ...
                        <div class="input-group input-group-md rounded-3 overflow-hidden border bg-light shadow-xs">
                            <span class="input-group-text bg-transparent border-0 text-primary ps-3"><i class="bi bi-ui-checks"></i></span>
                            <select id="tipoReporte" name="tipoReporte" class="form-select border-0 bg-transparent py-2.5 shadow-none cursor-pointer" required>
                                ...
                                <option value="mortalidad">Nacidos y muertos por tanque</option>
                                <option value="tanques">Tanques por zoocriadero</option>
                            </select>
                        </div>
                    </div>
...
(function () {
    ...
    const formulario = document.getElementById("formGenerarReporteZoo");
    ...
    formulario.addEventListener("submit", function (event) {
        ...
        resultado.innerHTML = `
            <div class="card border-0 shadow-sm rounded-4 p-5 text-center bg-white">
                <div class="py-4">
                    ...
                    <p class="text-muted small mb-0">Consultando la información solicitada en el sistema.</p>
                </div>
            </div>
        `;
        ...
        .then(function (texto) {
            ...
            if (respuesta.tipo === "seguimiento") mostrarSeguimiento(respuesta);
            else if (respuesta.tipo === "mortalidad") mostrarMortalidad(respuesta);
            else if (respuesta.tipo === "tanques") mostrarTanques(respuesta);
            else {
                alerta.innerHTML = `
                    <div class="alert alert-warning alert-dismissible fade show py-3 px-4 mb-4 border-0 shadow-sm rounded-4 bg-warning-subtle text-warning-emphasis" role="alert">
                        <div class="d-flex align-items-center gap-3">
            ...
            datos.forEach(function (fila) {
                html += `
                    <tr>
                        ...
                        <td class="py-3 fw-semibold text-dark">${escapar(fila.tipo)}</td>
                        <td class="py-3 text-muted">${escapar(fila.tanque)}</td>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light text-uppercase fs-7 text-secondary fw-bold">
                        <tr>
                            ...
                            <th class="py-3">Tanque</th>
                            <th class="py-3">Nacidos</th>
                            <th class="py-3">Muertos</th>
                            <th class="py-3">Responsable</th>
                            <th class="text-center py-3">Estado</th>
                        </tr>
                    </thead>
        ...
        if (datos.length === 0) {
            ...
            datos.forEach(function (fila) {
                html += `
                    <tr>
                        <td class="ps-4 py-3 text-muted">${escapar(fila.fecha)}</td>
                        <td class="py-3 fw-semibold text-dark">${escapar(fila.zoocriadero)}</td>
                        <td class="py-3 text-muted">${escapar(fila.tanque)}</td>
                        <td class="py-3"><span class="badge bg-success-subtle text-success-emphasis px-2.5 py-1">${escapar(fila.nacidos)}</span></td>
                        <td class="py-3"><span class="badge bg-danger-subtle text-danger-emphasis px-2.5 py-1">${escapar(fila.muertos)}</span></td>
                        <td class="py-3 text-muted">${escapar(fila.responsable)}</td>
                        <td class="text-center py-3">${mostrarEstado(fila.estado)}</td>
                    </tr>
                `;
        ...
        if (datos.length === 0) {
            ...
            datos.forEach(function (fila) {
                html += `
                    <tr>
                        <td class="ps-4 py-3 text-muted">${escapar(fila.fecha)}</td>
                        <td class="py-3 text-muted">${escapar(fila.zoocriadero)}</td>
                        <td class="py-3 fw-semibold text-dark">${escapar(fila.tanque)}</td>
                        <td class="py-3 text-muted">${escapar(fila.tipo)}</td>
                        <td class="py-3 text-muted">${escapar(fila.capacidad)} L</td>
                        <td class="text-center py-3">${mostrarEstado(fila.estado)}</td>
                    </tr>
                `;
            });
