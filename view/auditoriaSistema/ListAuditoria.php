    ...
    <div class="card-body bg-light bg-opacity-50 border-bottom py-3 px-4">  
      <form method="GET" action="<?php echo getUrl('Auditoria', 'Auditoria', 'listAuditoria') ?>" class="row g-2 align-items-end">  
        ...
        <div class="col-md-2">  
          ...
          <input type="date" class="form-control form-control-sm bg-white" name="desde" value="<?php echo htmlspecialchars($_GET['desde'] ?? ''); ?>">  
        </div>  
  
        <div class="col-md-2">  
          <label class="form-label text-muted small mb-1">Hasta</label>  
          <input type="date" class="form-control form-control-sm bg-white" name="hasta" value="<?php echo htmlspecialchars($_GET['hasta'] ?? ''); ?>">  
        </div>  
  
        <div class="col-md-3 ms-auto">  
          <label class="form-label text-muted small mb-1">Usuario</label>  
          <input type="text" class="form-control form-control-sm bg-white" name="usuario" placeholder="Buscar usuario..." value="<?php echo htmlspecialchars($_GET['usuario'] ?? ''); ?>">  
        </div>  
